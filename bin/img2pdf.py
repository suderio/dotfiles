...
pdf_stored_path = input(
    "Enter the name of you pdf file (please use backslash when typing in directory path):"
)

textFile_stored_path = path.join(
    path.dirname(pdf_stored_path),
    path.basename(pdf_stored_path).replace(".pdf", ".txt"),
)
...
with open(pdf_stored_path, "rb") as pdf_object:
    ...
    print(
        f"[+] Pdf Text has been extracted and written to {path.basename(textFile_stored_path)}"
    )
